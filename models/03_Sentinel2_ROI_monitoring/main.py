...
def Main(params):
    ...
    for i, y in enumerate(years):
        ...
        for index_file in index:
            ...
            with rasterio.open(ndvi[0]) as f:
                profile = f.profile
            ...
            final_file = os.path.join(root_folder, f"{y}_{index_file}.tiff")
            if os.path.exists(final_file):
                print(f"File {final_file} exists, skipping")
                continue
            # compute averages
            avg = compute_values(ndvi)
            print(avg.shape)
            #compute stdev
            stdev = compute_values(ndvi, method=np.ma.std)
            print(stdev.shape)
            res = roi_filter(clc_data, avg)
            res2 = roi_filter(clc_data, stdev)
            res = np.append(res[np.newaxis, :, :], res2[np.newaxis, :, :], axis=0)
            profile['count'] = 2
            with rasterio.open(final_file, "w", **profile) as f:  # write average and stdev to tiff as bands 1 and 2.
                f.write(res)
